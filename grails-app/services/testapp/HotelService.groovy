...
interface HotelService {
    ...
    @Query("SELECT ${h} FROM ${Hotel h} JOIN FETCH ${h.country} c WHERE c.id = $country AND lower(h.name)  LIKE   concat('%', lower($name), '%') ORDER BY h.stars DESC , h.name")
    List<Hotel> findAllByNameAndCountry(Long country, String name)
